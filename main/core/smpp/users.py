...
class Users(object):
    ...
    def create(self, data):
        """Create a User.
        Required parameters: username, password, uid (user identifier),
        ...
        set_ikeys(
            self.telnet,
            {"uid": uid, "gid": gid, "username": username, "password": password},
        )
    ...
    def partial_update(self, data, uid):
        ...
        * ["mt_messaging_cred", "authorization", "smpps_send", "False"]
        ...
        updates = data
        if not ((type(updates) is list) and (len(updates) >= 1)):  # noqa: E721
            raise JasminSyntaxError("updates should be a list")
        for update in updates:
            if not ((type(update) is list) and (len(update) >= 1)):  # noqa: E721
